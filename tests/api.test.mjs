...
test('callOpenAICompatible sends chat completions through the SillyTavern backend proxy', async () => {
  ...
  // 原版 ST 后端代理路径注入插件 UA（防提供商按 node-fetch 默认 UA 反欺诈拦截）
  assert.equal(body.custom_include_headers, [
...
test('host proxy config skips the User-Agent injection on TauriTavern', async () => {
  const calls = [];
  installBrowserHost(async (url, options) => {
    calls.push({ url, options });
    return jsonResponse({ response: JSON.stringify({ models: [{ name: 'grok-4' }] }) });
  });
  globalThis.__TAURITAVERN__ = { ready: true };

  await fetchModelList({ apiUrl: 'https://example-model-host.test/v1', apiKey: '' });

  assert.equal(calls.length, 1);
  const body = JSON.parse(calls[0].options.body);
  // TT 后端自带产品 UA 且 additional headers 最后应用，注入会把它顶掉
  assert.equal(body.custom_include_headers, '');
});
