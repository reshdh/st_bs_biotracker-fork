...
// 性欲的天花板：三条线里的 hard，跟 tools 侧共用 metabolism_config 那一份，
// 不再在这里另算一遍。旧版本这里是自己按月取整算 100→150，那会在阶段交界出台阶，
// 而且入盆后天花板要砸到 72，按月算的读法完全对不上。
function getLibidoCap(stage, profile = null) {
  const isTruePregnancy = ['孕早期', '孕中期', '孕晚期', '临产期', '逾期', '产兆前驱', '第一产程', '第二产程', '第三产程'].includes(stage);
  const engagedCount = (Array.isArray(profile?.pregnant?.fetuses) ? profile.pregnant.fetuses : [])
    .filter((fetus) => fetus?.engaged).length;
  return Math.round(getLibidoLines(
    stage,
    Number(profile?.pregnant?.effectivePregnantDays) || 0,
    engagedCount,
    isTruePregnancy,
  ).hard);
}
...
// lines 是「这个需求的尺子长什么样」，按 key 给：
//   urine → { urge, hard }（两条线，「满」落在 urge 上）
//   milk  → 数字，当前容量（随孕周从开奶周起往上长）
// 不给就退回默认的 100／150。乳意的容量必须从外面传进来：面板和引擎读同一份
// （metabolism_config 的 getMilkCapacityFromDays），否则「满了几成」两处会对不上。
function normalizeMetabolismNeed(key, metabolism = {}, lines = null) {
  // 尿意有两条线：urge（该去了）与 hard（收不住）。「满」落在 urge 上，
  // 图标的溢出段画的是 urge→hard 这段憋耐余量，所以不能拿单一 cap 等比缩放。
  // 便意有四条线（floor/urge/pain/cap），档位按这四条切（§二六档），也走同一份函数。
  const isUrine = key === 'urine' && lines && Number(lines.urge) > 0;
  const isStool = key === 'stool' && lines && Number(lines.urge) > 0;
  const milkCap = key === 'milk' && Number(lines) > 0 ? Number(lines) : 0;
  const urge = isUrine
    ? Math.round(Number(lines.urge))
    : isStool
      ? Math.round(Number(lines.urge))
      : Math.round(milkCap > 0 ? milkCap * (100 / 150) : 100);
  const cap = isUrine
    ? Math.round(Number(lines.hard))
    : isStool
      ? Math.round(Number(lines.cap))
      : Math.round(milkCap > 0 ? milkCap : 150);
  const value = key === 'flux'
    ? clampUiNumber(metabolism[key], -cap, cap, 0)
    : clampUiNumber(metabolism[key], 0, cap, 0);
  let level;
  if (key === 'flux') level = getDerivedFluxSummary(value, cap);
  else if (isUrine) level = getUrineLevel(value, urge, cap);
  else if (isStool) level = getStoolLevel(value, lines);
  else level = getMetabolismLevel(value, cap);
  return {
    key,
...
function getMetabolismSummary(metabolism = {}, immune = {}, derivedType = null, urineLines = null, milkCapacity = 0, stoolLines = null) {
  if (immune?.metabolism) return '代谢免疫';
  const linesOf = (key) => (key === 'urine' ? urineLines : key === 'milk' ? milkCapacity : key === 'stool' ? stoolLines : null);
  const needOf = (key) => normalizeMetabolismNeed(key, metabolism, linesOf(key));
  if (derivedType) {
    const exemptions = new Set(getDerivedTypeMetabolismExemptions(derivedType));
    const visible = (key) => (exemptions.has(key) ? null : needOf(key));
    return {
      flux: needOf('flux'),
...
const METABOLISM_DISPLAY_ORDER = Object.freeze(['urine', 'stool', 'hunger', 'sleep', 'milk']);
...
function renderMetabolismNeedIcon(item) {
  ...
  const value = Number(item?.value) || 0;
  // 满格线与硬上限分开：尿意的满格是「该去了」那条线，图标的溢出段画的是
  // 想去到收不住之间那段憋耐余量。其余需求两者仍是 100 与 cap。
  const cap = Number(item?.cap) > 0 ? Number(item.cap) : 150;
  const urge = Number(item?.urge) > 0 ? Number(item.urge) : 100;
  const fillValue = key === 'flux' ? Math.abs(value) : value;
  const fill = key === 'flux'
    ? Math.max(0, Math.min(100, (fillValue / cap) * 100))
    : Math.max(0, Math.min(100, (fillValue / urge) * 100));
  const overfill = key === 'flux' || cap <= urge
    ? 0
    : Math.max(0, Math.min(100, ((fillValue - urge) / (cap - urge)) * 100));
  const tone = key === 'flux'
  ...
  return {
    ...
    overview: {
      ...
      stats: [
        {
          label: '体力',
          // 内部浮点累积，面板取整；软顶是「现在能恢复到哪儿」的刻度，读法 68 / 95（体质 125）。
          value: Math.round(Number(base.vitality) || 0),
          cap: VITALITY_CAPS[Math.max(1, Math.min(7, Math.round(Number(base.vitalityLevel) || 4)))] || 125,
          softCap: getVitalitySoftCap(profile, VITALITY_CAPS[Math.max(1, Math.min(7, Math.round(Number(base.vitalityLevel) || 4)))] || 125),
        },
        // 档名与标签走 badge：绝对值的分母（天花板）随孕周一直在动，
        // 入盆那一下会从 71/104 跳成 71/72，光看绝对值读不出她的状态。
        // badge 读的是 getLibidoView，跟发给模型的是同一份。
        {
          label: '性欲',
          value: Number(profile.metabolism?.libido) || 0,
          cap: getLibidoCap(stage, profile),
          badge: getLibidoView(profile).text,
        },
        {
          label: '情压',
      ...
      metabolismSummary: getMetabolismSummary(
        profile.metabolism,
        immune,
        base.derivedType,
        // 尿意两条线：urge（该去了）与 hard（收不住）。
        ((engagedCount, progress) => ({
          urge: getUrineUrgeCap(stage, engagedCount, progress),
          hard: getUrineHardCap(stage, engagedCount, progress),
        // eslint-disable-next-line no-sequences
        }))((Array.isArray(pregnant.fetuses) ? pregnant.fetuses : []).filter((fetus) => fetus?.engaged).length,
        (() => {
          let deepest = 0;
          ...
          return Math.max(0, Math.min(1, (deepest - 40) / 60));
        })()),
        // 乳意的容量随孕周从开奶周起往上长，所以「满了几成」不能拿写死的 150 算。
        getMilkCapacityFromDays(
          bio.milkGate,
          pregnant.effectivePregnantDays,
          ['孕早期', '孕中期', '孕晚期', '临产期', '逾期', '产兆前驱', '第一产程', '第二产程', '第三产程'].includes(stage),
        ),
        // 便意四条线（floor/urge/pain/cap）——面板与引擎调同一份函数取线，
        // 否则「满了几成」两处对不上（坑#2）。
        getStoolLines(
          stage,
          (Array.isArray(pregnant.fetuses) ? pregnant.fetuses : []).filter((fetus) => fetus?.engaged).length,
    ...
    pregnancy: {
      ...
      effectiveLaborHours: Number(pregnant.effectiveLaborHours) || 0,
      ...
      engagedDays: Number(pregnant.engagedDays) || 0,
      // 裸值。gauges 里那份是 {label,value,cap} 显示对象，按 label 翻数组太脆。
      uterinePressure: Number(base.uterinePressure) || 0,
      naturalBirthExperience: Number(experience.naturalBirthExperience) || 0,
      complication: String(pregnant.complication || '').trim(),
      prodromalEpisodeHours: Number(pregnant.prodromalEpisodeHours) || 0,
...
function renderProgressList(items) {
  return items
    .map((item) => {
      ...
      // badge 是给性欲用的：绝对值那一栏（71 / 104）分母一直在动，孕周一变就跳数，
      // 所以档名与附加标签另挂一块。其余三项不传 badge，渲染结果完全不变。
      const badge = String(item.badge || '').trim();
      const badgeHtml = badge
        ? `<span class="bs-bt-track-progress-badge">${escapeHtml(badge)}</span>`
        : '';
      // 软顶刻度（体力专用）：全宽是体质上限，填充是当前值，软顶画一道线——
      // 「还没花掉，但花掉就回不来了」的那条线。没传 softCap 的项完全不变。
      const softCap = Number(item.softCap);
...
// ⚠️ 这里原来有个 toggleSelectedTrackSuppress（压抑芯片的 ON/OFF），已整个删掉。
// 那个按钮让用户手动决定小中大，而档位该由剧情里发生了什么决定；
// 而且它跟自己的说明书方向相反（按住闸 → 峰值更低 → 攒得更少 → 去了还更小）。
// 完整理由见 scripts/libido_config.js 里「压抑芯片：已整个删掉」那一段。
...
function validateManualCharacterState(next, currentName) {
  ...
  const numericPaths = [
    ...
    // 性欲已搬进 metabolism。base.libido 仍留在这张表上是给旧档兜底：
    // 手改全量 JSON 时那个键可能还在，normalizeCharacterPsychologyState 会把它搬走。
    ['profile', 'base', 'libido'],
    ['profile', 'metabolism', 'libido'],
