...
import { applyToolCallsResult, getLibidoView, TOOL_DEFINITIONS } from './tools.js';
import { applyLibidoAnalgesia } from './libido_config.js';
import {
  buildRecentMessages,
  buildSignature,
  cloneValue,
...
function getPromptFacingLaborState(base = {}, pregnant = {}) {
  ...
  const rawPain = Number.isFinite(Number(pregnant.laborPain)) ? Number(pregnant.laborPain) : 0;
  return {
    ...
    // 大档高潮的镇痛窗口（TASK-05 拍板 #7）：打折挂在读侧——产程每轮都会
    // updateLaborPain 重写痛值，写侧打折会被下一次推进冲掉。
    laborPain: applyLibidoAnalgesia(pregnant.libidoAnalgesia, rawPain),
  };
}
...
function buildPromptFacingCharacterState(item, diaryLimit = 0) {
  ...
  // 性欲发的是**视图不是数字**：档名、占满线几成、附加标签、身体状态的定性说法。
  // 闸、电荷、习惯化、不应剩余（都在 profile.libido 上）一概不发——
  // 那四样是内部量，报出去等于把「还差多少」告诉模型，而口径 §十二 明令不给：
  // 知道还差多少，模型就会规划「再几分钟就到」，那是叙述者的条理冒充身体。
  //
  // ⚠️ profile.libido 这个顶层字段必须删掉。它跟 metabolism.libido 是两回事，
  // 名字又像，漏删就等于把隐藏量整份发出去了。
  //
  // hard（天花板）也不发：那是给调试面板的，而且它一直在动（入盆后只剩 72），
  // 发出去只会让模型拿绝对值去推算余量。
  const { level, percent, tag, phase } = getLibidoView(item?.profile || {});
  const libidoView = { level, percent, tag, phase };
  delete profile.libido;
  ...
  if (base.derivedType) {
    ...
    profile.metabolism = {
      ...
      ...includeNeed('sleep'),
      ...includeNeed('milk'),
      ...(exemptions.has('libido') ? {} : { libido: libidoView }),
    };
  } else {
    profile.metabolism = {
  ...
  // 代谢免疫：那七项整块不发（她不饿不困不想上厕所，模型不必知道有这些东西）。
  // **但性欲照发**——那个开关不管她起不起来。整块删掉的话模型看不见她到哪一步了，
  // 于是他做了半天她在正文里毫无反应，而引擎那边其实一直在涨。
  if (immune.metabolism) {
    // 种族抵免掉性欲的（若有）连这一格也没有：那是「这个种族没有这一项」。
    const keptLibido = profile.metabolism?.libido;
    if (keptLibido === undefined) delete profile.metabolism;
    else profile.metabolism = { libido: keptLibido };
  }
...
// 导出仅供测试：这一层决定「哪些字段会进提示词」，而漏删一个隐藏量
// 不会报错、只会静默把它发给模型，所以必须能被断言钉住。
export { buildTrackerStateView as __buildTrackerStateViewForTest };
...
async function processTrackerMessage(ctx, settings, chatState, deps, reason, messageIndex) {
  ...
  applyToolCallsResult(ctx, result, { messageIndex });
