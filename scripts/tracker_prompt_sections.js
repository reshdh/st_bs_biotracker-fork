...
// Stable section IDs own inclusion; titles and section order can change independently.
export const TRACKER_VARIABLE_GUIDE_SECTIONS = Object.freeze([
  Object.freeze({ id: 'intro', text: [
    '以下是角色状态变量的语义说明，供你理解 existing_state 中的字段，不是要求你原样输出这些字段。',
    '',
  ].join('\n') }),
  Object.freeze({ id: 'structure', text: [
    '[总结构]',
  ...
  Object.freeze({ id: 'base', text: [
    ...
    '- eggs: 当前可受精卵子数。',
    '- ovaryStatusText: 系统附带的卵巢/排卵状态一句话描述（如「卵泡发育中，本周期尚未排卵」「已排出 2 颗成熟卵子，尚可受精」），只读派生字段，与卵子数、受精状态同源，叙事时直接采用其口径。',
    '- uterusStatusText: 系统附带的子宫状态一句话描述（如「子宫内膜增厚松软，静候着床结果」「规律宫缩进行中，宫颈口持续扩张」），只读派生字段，按当前阶段给出，叙事时直接采用其口径。',
    '- fertilityChanceText: 系统附带的受孕几率描述（如「0%，体内无精液残留」「约 85%，易孕峰值」「已受精，精子来自某某」），只读派生字段，口径值而非实时计算，叙事时直接采用。',
    ...
    '- psyStress: 情压/精神压力，慢变量。事件压力会随时间自动回落到她的本性基线（等级初始值，约 2 点/小时），所以不必每轮手动报减；只在正文明确写了平复情节时报负增量（被安抚 -5~10、痛哭发泄后 -10、重大释怀 -15~20）。报正增量只按事件量级给小整数：轻 3~5（被说了一句、小尴尬）／中 8~12（争吵、惊吓、当众羞辱）／强 15~20（重大打击、创伤事件），单次不超过 30；日常闲聊的小情绪不报。',
    '- vitalityLevel / psyStressLevel: 个体等级，决定对应数值上限与体质倾向。psyStressLevel 同时是情压基线的依据：等级高的人平静下来后也停在一个较高的本性水平，不会回到接近 0。',
    '- vitalityLevelText / psyStressLevelText: 系统额外附带的等级文字说明，方便直接理解体质与精神倾向。',
    '',
  ].join('\n') }),
  Object.freeze({ id: 'pregnant', text: [
    '[pregnant]',
  ...
  Object.freeze({ id: 'experience', text: [
    '[experience]',
    '- 记录第一次对象、最近对象、情感/婚姻对象，以及怀孕、分娩、流产等经历次数。',
    '- 这类字段偏长期记录，通常只在剧情明确成立时才需要更新。',
    '',
  ].join('\n') }),
  Object.freeze({ id: 'psychology', text: [
    '[psychology]',
  ...
  Object.freeze({ id: 'children', text: [
    '[children]',
  ...
  Object.freeze({ id: 'metabolism', text: [
    '[metabolism]',
  ...
  Object.freeze({ id: 'wardrobe', text: [
    '[wardrobe / outfit]',
  ...
  Object.freeze({ id: 'descriptions', text: [
    '[descriptions]',
