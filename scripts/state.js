...
import {
  BASE_METABOLISM_CAP,
  LEGACY_EXCRETION_SPLIT,
  METABOLISM_KEYS,
  METABOLISM_PATCH_KEYS,
  MILK_GATE_KEYS,
...
export function normalizeCharacterPsychologyState(characterState) {
  ...
  if (metabolism && typeof metabolism === 'object' && !Array.isArray(metabolism)) {
    // 曾经被合并的 excretion 要拆回尿意与便意。合过的值恢复不了真值，
    // 只能按 6:4 近似分摊一次；此后两个键各走各的孕期修正（方向相反）。
    if (metabolism.excretion !== undefined) {
      const merged = sanitizeNumber(metabolism.excretion, { min: 0, max: BASE_METABOLISM_CAP }) ?? 0;
      if (metabolism.urine === undefined) metabolism.urine = merged * LEGACY_EXCRETION_SPLIT.urine;
      if (metabolism.stool === undefined) metabolism.stool = merged * LEGACY_EXCRETION_SPLIT.stool;
      delete metabolism.excretion;
    }
    if (metabolism.urine === undefined) metabolism.urine = 0;
    if (metabolism.stool === undefined) metabolism.stool = 0;
    // 臭意（odor）与伴意（companionship）已整个删掉（TASK-04）：
    // 八键旧档读进来时把这两个残留键静默丢掉，不报错。
    delete metabolism.odor;
    delete metabolism.companionship;
    // 性欲从 base 搬进 metabolism：要用 addMetabolismValue 那套分档权重
    //（「这一份算哪一侧」）就必须是代谢键。旧档的值原样搬过来，不重算。
    if (metabolism.libido === undefined) {
      const legacy = characterState.profile.base?.libido;
      metabolism.libido = legacy === undefined
        ? 0
        : (sanitizeNumber(legacy, { min: 0, max: BASE_METABOLISM_CAP }) ?? 0);
    }
  }
  // 搬完就把 base 上那个摘掉，避免两处各存一份、之后读到旧值。
  if (characterState.profile.base && characterState.profile.base.libido !== undefined) {
    delete characterState.profile.base.libido;
  }
  ...
  if (metabolism && typeof metabolism === 'object' && !Array.isArray(metabolism)) {
    // 尿意的上限随孕期阶段与入盆状态压缩，不能在这里硬写 150：
    // 加载时按 150 放行、运行时按 70 计算，两套标准之间的差会让档位显示错一大截。
    const stage = String(characterState.profile.base?.stage || '');
    const engagedCount = Array.isArray(pregnant?.fetuses)
      ? pregnant.fetuses.filter((fetus) => fetus?.engaged).length
      : 0;
    for (const key of METABOLISM_KEYS) {
      if (metabolism[key] === undefined) continue;
      // 乳意的容量随发育在 18~150 之间走，这里仍按 150 放行：运行时的 cap 一定 ≤150，
      // 写入都过 addMetabolismValue 会再夹一次，所以放宽不会漏进越界值。
      // 这里不算乳意容量是因为它要读 bio.milkGate，而 bio 在这一步还没归一化完。
      const baseCap = key === 'urine' ? getUrineHardCap(stage, engagedCount) : BASE_METABOLISM_CAP;
      metabolism[key] = sanitizeNumber(metabolism[key], { min: 0, max: baseCap }) ?? 0;
    }
    if (metabolism.flux !== undefined) {
      const flux = Number(metabolism.flux) || 0;
      metabolism.flux = sanitizeNumber(flux, { min: -BASE_METABOLISM_CAP, max: BASE_METABOLISM_CAP }) ?? 0;
    }
  }
  // stool 子对象归一化（TASK-01 双轴重写）：
  // 旧档可能带着五个已删的死标志位（ironSupplement/dehydrated/squatPosture/
  // fiberIntake/hydrated），在这里清掉；新字段缺失的填默认值。
  // difficulty 缺失时填阶段底噪——tools.js 的 applyStoolDifficultyDrift 会重算。
  if (characterState.profile.stool && typeof characterState.profile.stool === 'object' && !Array.isArray(characterState.profile.stool)) {
    const oldStool = characterState.profile.stool;
    const cleaned = {
      difficulty: sanitizeNumber(oldStool.difficulty, { min: 0, max: 100 }) ?? undefined,
      failDays: sanitizeInteger(oldStool.failDays, { min: 0, max: 99 }) ?? 0,
      lastAttemptMinutes: sanitizeNumber(oldStool.lastAttemptMinutes, { min: 0, max: 999999 }) ?? 0,
      wateryValue: sanitizeNumber(oldStool.wateryValue, { min: 0, max: 10 }) ?? 0,
      wateryPrelude: sanitizeNumber(oldStool.wateryPrelude, { min: 0, max: 240 }) ?? 0,
      wateryWaves: sanitizeInteger(oldStool.wateryWaves, { min: 0, max: 99 }) ?? 0,
    };
    if (cleaned.difficulty === undefined) delete cleaned.difficulty;
    if (Array.isArray(oldStool.gasBoosts)) {
      cleaned.gasBoosts = oldStool.gasBoosts
        .map((b) => ({
          mult: sanitizeNumber(b?.mult, { min: 0, max: 100 }) ?? 1,
          expiresAt: sanitizeNumber(b?.expiresAt, { min: 0, max: 999999 }) ?? 0,
        }))
        .filter((b) => b.mult !== null);
    }
    characterState.profile.stool = cleaned;
  }
  ...
  return value
    ...
    .map((item) => {
      ...
      const numberFields = {
        ...
        // 下降度：胎头往骨盆里走了多深。engaged 由它派生，不再是独立的开关。
        descent: { min: 0, max: 100 },
      };
      // 这个函数是重建式白名单（fetus 从空对象起塞），不列出的字段会被静默丢掉。
      // engaged 一旦漏掉，尿意容量会在某次追踪后无声弹回未入盆的档，查不出原因。
      if (item.engaged !== undefined) fetus.engaged = Boolean(item.engaged);
...
export function sanitizeProfilePatch(profilePatch) {
  ...
  // 这张白名单要跟 tools.js 里写进 profile.cooldown 的键保持一致：
  // 不在名单上的会被静默丢掉，于是「已经出声过」这件事在存读档之后失忆，
  // 同一次漏尿／失禁重载后会再响一遍。
  const cooldown = sanitizeObjectPatch(profilePatch.cooldown, [
    'orgasmOvulationUsed',
    ...
    'urineLeakWarned',
    'urineIncontinenceWarned',
    'urineIncontinenceAt',
    'pregnancySymptomActive',
  ], {
    ...
    urineLeakWarned: (value) => Boolean(value),
    urineIncontinenceWarned: (value) => Boolean(value),
    urineIncontinenceAt: (value) => sanitizeNumber(value, { min: 0, max: 999999 }),
  ...
  const base = sanitizeObjectPatch(
    ...
    ],
    {
      ...
      vitality: (value) => sanitizeNumber(value, { min: 0, max: 200 }),
  ...
  const pregnant = sanitizeObjectPatch(
    ...
    ['pregnantDays', 'effectivePregnantDays', 'laborHours', 'effectiveLaborHours', 'laborPhase', 'laborFetusIndex', 'laborPain', 'prodromalOriginStage', 'prodromalRemainingHours', 'prodromalDelayProgressHours', 'fetusesCount', 'fetalEnergyDrain', 'nutrition', 'symptomReliefPending', 'complication', 'engagedDays', 'prodromalEpisodeHours', 'prodromalEpisodeCooldownHours', 'prodromalEpisodeCount', 'morningSicknessResolved', 'prolonged', 'libidoAnalgesia', 'fetuses'],
    {
      ...
      // 孕吐好转覆写口（TASK-05 拍板 #4）：一次性布尔，丢了会让已报过的角色
      // 重载后掉回 0.8 谷底曲线。
      morningSicknessResolved: (value) => Boolean(value),
      // 延产声明（TASK-02 拍板）：丢了会让延产角色重载后掉回自然逾期线。
      prolonged: (value) => Boolean(value),
      // 大档高潮的镇痛窗口剩余分钟（TASK-05 拍板 #7）：丢了会把窗口重置成
      // 「永远开着」——读侧只看这个数在不在。
      libidoAnalgesia: (value) => sanitizeNumber(value, { min: 0, max: 9999 }),
      ...
      // 病理并发症：胎盘前置、宫颈松弛这类。空 = 健康 = 子宫压力再高也只到先兆征象。
      // 只有填了这一项，压力才重新有能力导致流产或提前发动。
      complication: sanitizeString,
      // 胎头入盆已持续多少天。独立于孕周：浮回去就清零。
      engagedDays: (value) => sanitizeNumber(value, { min: 0, max: 9999 }),
      // 可逆前驱发作：本次还剩几小时、散掉后的间隔、以及一共来过几次。
      // 丢了它们，一次正在进行的发作会在重载后变成「没发生过」。
      prodromalEpisodeHours: (value) => sanitizeNumber(value, { min: 0, max: 9999 }),
      prodromalEpisodeCooldownHours: (value) => sanitizeNumber(value, { min: 0, max: 9999 }),
      prodromalEpisodeCount: (value) => sanitizeInteger(value, { min: 0, max: 999 }),
  ...
  const bio = sanitizeObjectPatch(
    ...
    ],
    {
      ...
      // 乳腺出口的松紧，建卡时定，之后不随行为改变。不认的值一律回落到中档。
      milkGate: (value) => (MILK_GATE_KEYS.includes(String(value || '').trim()) ? String(value).trim() : '中'),
      // 体质档（多奶/普通/少奶）——查表不掷骰，每胎一致。不认的值回落普通。
      milkConstitution: (value) => (MILK_CONSTITUTION_KEYS.includes(String(value || '').trim()) ? String(value).trim() : '普通'),
    },
  );
  ...
  const metabolism = sanitizeObjectPatch(profilePatch.metabolism, METABOLISM_PATCH_KEYS, {
    urine: (value) => sanitizeInteger(value, { min: 0, max: 200 }),
    stool: (value) => sanitizeInteger(value, { min: 0, max: 200 }),
  ...
  // stool 子对象是便意双轴的隐藏持久状态（TASK-01）：difficulty（阻力值）、
  // failDays（纯 notify 演示字段）、lastAttemptMinutes（冷却窗口起点）、
  // wateryValue（隐藏稀值）、wateryPrelude（窜稀前摇分钟）、wateryWaves（波次）、
  // gasBoosts（排气临时加成列表，带过期清扫）。
  // 不在名单上的字段会被丢掉，于是「刚失败过、冷却还没过」在存读档之后失忆。
  const stoolState = sanitizeObjectPatch(profilePatch.stool, [
    'difficulty',
    'failDays',
    'lastAttemptMinutes',
    'cooldownUntil',
    ...
    difficulty: (value) => sanitizeNumber(value, { min: 0, max: 100 }),
    failDays: (value) => sanitizeInteger(value, { min: 0, max: 99 }),
    lastAttemptMinutes: (value) => sanitizeNumber(value, { min: 0, max: 999999 }),
    cooldownUntil: (value) => sanitizeNumber(value, { min: 0, max: 999999 }),
    wateryValue: (value) => sanitizeNumber(value, { min: 0, max: 10 }),
    wateryPrelude: (value) => sanitizeNumber(value, { min: 0, max: 240 }),
    wateryWaves: (value) => sanitizeInteger(value, { min: 0, max: 99 }),
    gasBoosts: (value) => {
      if (!Array.isArray(value)) return null;
      return value.map((b) => ({
        mult: sanitizeNumber(b?.mult, { min: 0, max: 100 }) ?? 1,
  ...
  // urine 子对象：排尿状态（voidsToday 趟数、holding 憋着、voidCooldownUntil 排不出来冷却）。
  // voidCooldownUntil 不在名单上 → 存读档后失忆 → 刚起不了流紧接着又能试。
  // senseEffects 是栏二临时偏移列表（TASK-02 拍板）、sweat 是出汗事件——丢了会丢状态。
  const urineState = sanitizeObjectPatch(profilePatch.urine, [
    'voidsToday',
    'holding',
    'voidCooldownUntil',
    'senseEffects',
    ...
    voidsToday: (value) => sanitizeInteger(value, { min: 0, max: 999 }),
    holding: (value) => Boolean(value),
    voidCooldownUntil: (value) => sanitizeNumber(value, { min: 0, max: 999999 }),
    senseEffects: (value) => Array.isArray(value) ? value.filter((e) => e && typeof e === 'object') : undefined,
    sweat: (value) => (value && typeof value === 'object' ? value : undefined),
  });
  // milk 子对象：乳腺堵住状态（TASK-06 补序列化）。
  // duct=出口通着/结痂、blockHours=堵的计数器、quietHours=干在原地的时长。
  // reflectReady=排乳反射是否已建立、firstSupplyMinutes=首次有奶的时刻（反射计时起点）。
  // 不在名单上 → 存读档后 duct 和 blockHours 丢，堵奶计数器归零，反射状态丢失。
  const milkState = sanitizeObjectPatch(profilePatch.milk, [
    'duct',
    'blockHours',
    'quietHours',
    'reflectReady',
    ...
    duct: (value) => (String(value || '').trim() === '结痂' ? '结痂' : '通着'),
    blockHours: (value) => sanitizeNumber(value, { min: 0, max: 9999 }),
    quietHours: (value) => sanitizeNumber(value, { min: 0, max: 9999 }),
    reflectReady: (value) => Boolean(value),
    firstSupplyMinutes: (value) => sanitizeNumber(value, { min: 0, max: 999999 }),
  });
  const result = {};
  ...
  if (stoolState) result.stool = stoolState;
  if (urineState) result.urine = urineState;
  if (milkState) result.milk = milkState;
