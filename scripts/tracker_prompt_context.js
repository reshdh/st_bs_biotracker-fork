...
export const TRACKER_VARIABLE_GUIDE_PROMPT = [
  ...
  '- 体力五档（按当前值占体质上限的百分比）：' +
    '①充沛（≥80%）：身体轻盈无负担，精力充沛，承受高强度活动余量充足；' +
    '②倦意（50~79%）：活动中尚无明显感觉，停歇时疲惫感才逐渐显现，倾向寻找倚靠休息；' +
    '③吃力（25~49%）：动作出现细微变形（如上下楼扶栏、起蹲扶膝），哈欠增多，言语简短，虽感疲惫但仍能勉力支撑；' +
    '④脱力（<25%）：肢体发软不听使唤，动作迟缓，猛起易发黑，手心发虚汗，能躺不坐；' +
    '⑤耗竭（≤8%）：意识模糊昏沉，言语难以成句，呼唤反应迟钝，几乎仅凭生理本能支撑。',
  ...
  '- 无（<25%）：平稳无欲。局部干燥无胀坠，无生理唤起迹象，日常行为完全不带性冲动色彩。',
  '- 低/微热（25~49%）：微有余温。仅有微弱生理存在感，偶有极轻微湿润，极易消退，不影响日常专注。',
  '- 中/动情（50~74%）：初起情动。局部微胀并自发潮润，会下意识并腿或调整坐姿缓解，适度活动后可自然平复。',
  '- 高（75~99%）：强烈克制。充血肿胀与敏感度显著上升，衣物摩擦即可产生刺激；举止紧绷戒备，言语迟滞，强自遮掩。',
  '- 满（100~124%）：充盈临界。局部充血发烫、高度润滑并伴有节律性抽紧；腰肢发软，呼吸紊乱，难以维持自制。',
  '- 爆/发情（≥125%）：深度发情/过载。充血剧烈至酸胀，身体颤软无力，感官完全被情欲占据，已无法掩饰生理反应。',
  '- tag 与 level 并存：绝顶前夕/临界（gate≥75 逼近高潮）、高潮余韵（停手后仍处于痉挛与心跳过速期）、不应·轻/软/躲（高潮后敏感不应期各阶段反应）。',
