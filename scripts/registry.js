...
export function buildRegistrySystemPrompt(settings, options = {}) {
  ...
  const prompt = [
    ...
    '如果资料不足，可以省略字段或给 null；不要为了凑完整而编造。metabolism 除外——即使剧情没明说，也应按开局时机的常识推断需求初值（见下方 metabolism 说明）。',
    ...
    '- base.age: 角色年龄',
    '- metabolism.libido: 初始性欲。它有三条线（起点／满线／天花板）随孕期走，都由系统按天算：什么都不做时值会落回起点而不是 0，过了满线才算她自己知道想要了。非妊娠是 18／100／150，孕晚期起点抬高、满线与天花板一起降，胎头入盆后余量只剩 8。若角色开场就在发情、催情、强欲状态，可给较高值。（写成 base.libido 也收，会自动搬到这里。）',
    ...
    '示例：',
    '- 人类少女(排卵期): {"base":{"race":"人类","stage":"排卵期","vitalityLevel":4,"psyStressLevel":4,"age":18,"uterinePressure":0},"metabolism":{"libido":12,"urine":45,"hunger":30,"sleep":10}}',
    '- 混血(月经期): {"base":{"race":"天使x恶魔","stage":"月经期","vitalityLevel":5,"psyStressLevel":3,"age":25,"uterinePressure":3},"metabolism":{"libido":35,"urine":0,"hunger":0,"sleep":80}}',
...
const METABOLISM_FIELDS = ['urine', 'stool', 'hunger', 'sleep', 'milk', 'libido', 'flux'];
...
function sanitizeRegistryProfile(profile, baseProfile) {
  ...
  if (profile.base && typeof profile.base === 'object' && !Array.isArray(profile.base)) {
    ...
    // 性欲已搬进 metabolism。注册时仍收 base.libido（旧卡、旧提示词都这么写），
    // 但落到 metabolism.libido 上。两边都给的话以 metabolism 为准。
    if (profile.base.libido !== undefined) {
      const libido = sanitizeMeter(profile.base.libido, { min: 0, max: 150 });
      if (libido !== null) legacyBaseLibido = libido;
    }
  ...
  if (Object.keys(metabolism).length > 0 || legacyBaseLibido !== null) {
    ...
    // 旧写法 base.libido 落到这里。metabolism.libido 已经给了就不覆盖。
    if (legacyBaseLibido !== null && nextMetabolism.libido === undefined) {
      nextMetabolism.libido = legacyBaseLibido;
    }
